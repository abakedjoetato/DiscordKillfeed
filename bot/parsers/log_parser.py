...
class LogParser:
    ...
    async def get_sftp_log_content(self, server_config: Dict[str, Any]) -> Optional[str]:
        """Get log content from SFTP server"""
        try:
            # SFTP configuration
            ...
            try:
                file_content = sftp.open(remote_path, 'r').read()
                sftp.close()
                ssh.close()
                return file_content
    ...
    async def get_dev_log_content(self) -> Optional[str]:
        """Get log content from dev_data directory"""
        try:
            log_path = Path('./dev_data/logs/Deadside.log')
            
            if not log_path.exists():
                logger.warning("No log file found in dev_data/logs/")
                return None
