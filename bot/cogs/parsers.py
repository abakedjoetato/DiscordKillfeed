...
class Parsers(commands.Cog):
    ...
    async def parser_refresh(self, ctx: discord.ApplicationContext, server_id: str = "default"):
        ...
        try:
            ...
            if hasattr(self.bot, 'historical_parser') and self.bot.historical_parser:
                try:
                    await self.bot.historical_parser.refresh_historical_data(guild_id, server_id)
                    
                    embed = discord.Embed(
                        title="🔄 Data Refresh Initiated",
                        description=f"Historical data refresh started for server **{server_id}**",
                        color=0x00FF00,
                        timestamp=datetime.now(timezone.utc)
                    )
